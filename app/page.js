...
export default function Page(){
 ...
 return <main>
  ...
  {!rows.length?<section className="empty"><FileSpreadsheet size={52}/><h2>Belum ada data</h2><p>Upload file GRIR untuk mulai melihat monitoring.</p><label className="primary">Pilih File Excel<input type="file" accept=".xlsx,.xls" onChange={onFile}/></label></section>:<>
   ...
   <section className="cards"><Card kind="total" title="TOTAL GRIR" value={money(total)} sub={`${filteredRows.length.toLocaleString('id-ID')} transaksi`}/><Card kind="due" title="DUE" value={money(dueData.find(x=>x[0]==='Jatuh Tempo')?.[1]||0)} sub="Jatuh tempo"/><Card kind="notdue" title="NOT DUE" value={money(dueData.find(x=>x[0]==='Belum Jatuh Tempo')?.[1]||0)} sub="Belum jatuh tempo"/><Card kind="vendor" title="VENDOR" value={unique(filteredRows,'vendor').length} sub="Total vendor"/></section>
...
function Card({kind,title,value,sub}){const icons={total:"◉",due:"◷",notdue:"◷",vendor:"●"};return <div className={`card card-${kind||"default"}`}><div className="cardtop"><span className="cardicon">{icons[kind]||"•"}</span><span className="cardtitle">{title}</span></div><strong>{value}</strong><small>{sub}</small><div className="cardtrend"><b>▲ 0%</b><span>vs periode sebelumnya</span></div></div>}
